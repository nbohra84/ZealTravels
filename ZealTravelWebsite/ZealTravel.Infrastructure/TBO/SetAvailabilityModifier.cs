...
{
    public class SetAvailabilityModifierOneWay
    {
        ...
        private string Cabin{get; set; }
        private string Origin{get; set; }
        private string Destination{get; set; }
        private string Sector{get; set; }
        private string FltType{get; set; }

        //-----------------------------------------------------------------------------------------------
        public SetAvailabilityModifierOneWay(string Supplierid, string Searchid, string Companyid, string Origin, string Destination, string Cabin, string Sector, string FltType, string Tokenid)
        {
        ...
        public DataTable FlightModifier(string Response)
        {
            DataTable dtFlights = Schema.SchemaFlights;
            ...
            try
            {
                ...
                XmlNodeList nodes = root.SelectNodes("Results/Results");
                ...
                {
                    Refid++;
                    FirstFilter(node.OuterXml, Refid, dtTemp);
                }
                //-------------------------------------------------------------------------------------------------
                foreach (DataRow dr in dtTemp.Rows)
                {
                    SecondFilter(dr, dtFlights);
                }
            }
            catch (Exception ex)
            {
                errorMessage = ex.Message;
                //DBCommon.Logger.dbLogg(Companyid, 0, "FlightModifier", "air_tbo-SetAvailabilityModifierOneWay", Supplierid, Searchid, errorMessage);
            }

            if (dtFlights != null && dtFlights.Rows.Count > 0)
            {
                GetFlightModifierFunctions.SetJourneyTimeforConnectionFlights(dtFlights);
            }

            return dtFlights;
        }
        //--------------------------------------------------------------------------------------------------------------------------------------------
        private void SecondFilter(DataRow dr, DataTable dtFlights)
        {
            try
            {
                XmlDocument xmlflt = new XmlDocument();
                ...
                {
                    ThirdFilter(node.OuterXml, dr, dtFlights);
                }
            }
            catch (Exception ex)
            {
                errorMessage = ex.Message;
                //DBCommon.Logger.dbLogg(Companyid, 0, "SecondFilter", "air_tbo-SetAvailabilityModifierOneWay", Supplierid, Searchid, errorMessage);
            }
        }
        private void ThirdFilter(string Segments, DataRow dr, DataTable dtFlights)
        {
            try
            {
                ...
                if (AirlineCode.Equals("I5") && Baggage.IndexOf("0 K") != -1 && Sector.Equals("D"))
                {
                    Baggage = "15 KG";
                }
                else
                {
                    if (Baggage.IndexOf("0 K") != -1 && (dr["AirlineRemark"].ToString().ToUpper().IndexOf("15KG") != -1 || dr["AirlineRemark"].ToString().ToUpper().IndexOf("15 KG") != -1))
                    {
                        Baggage = "15 KG";
                    }
                    else if (Baggage.IndexOf("0 K") != -1 && (dr["AirlineRemark"].ToString().ToUpper().IndexOf("1PC") != -1 || dr["AirlineRemark"].ToString().ToUpper().IndexOf("1 PC") != -1))
                    {
                        Baggage = "1 PC";
                    }
                }
            ...
            {
                errorMessage = ex.Message;
                //DBCommon.Logger.dbLogg(Companyid, 0, "ThirdFilter", "air_tbo-SetAvailabilityModifierOneWay", Supplierid, Searchid, errorMessage);
            }
        }
        //--------------------------------------------------------------------------------------------------------------------------------------------
        private void FirstFilter(string OuterXml, Int32 RefID, DataTable dtTemp)
        {
            ...
            {
                errorMessage = ex.Message;
                //DBCommon.Logger.dbLogg(Companyid, 0, "FirstFilter", "air_tbo-SetAvailabilityModifierOneWay", OuterXml, Searchid, errorMessage);
            }
        }
    }
}
