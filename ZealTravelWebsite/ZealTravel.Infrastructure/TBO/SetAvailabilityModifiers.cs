...
{
    public class SetAvailabilityModifiers
    {
        ...
        //-----------------------------------------------------------------------------------------------
        public SetAvailabilityModifiers(string Supplierid, string Searchid, string Companyid, string Origin, string Destination, string Cabin, string Sector, string Tokenid)
        {
        ...
        public DataTable FlightModifier(string Response, bool IsCombi)
        {
            ...
            catch (Exception ex)
            {
                ...
                //DBCommon.Logger.dbLogg(Companyid, 0, "FlightModifier", "air_tbo-SetAvailabilityModifiers", Supplierid, Searchid, errorMessage);
            }

            if (dtFlights != null && dtFlights.Rows.Count > 0)
            {
                GetFlightModifierFunctions.SetJourneyTimeforConnectionFlights_RT(dtFlights);
        ...
        private void ThirdFilter(string Segments, DataRow dr, DataTable dtFlights)
        {
            ...
            foreach (XmlNode node in nodesSegments)
            {
                FourthFilter(node.OuterXml, dr, dtFlights);
            }
        }
        private void FourthFilter(string Segments, DataRow dr, DataTable dtFlights)
        {
            try
            {
                XmlDocument xmlflt = new XmlDocument();
            ...
            {
                //DBCommon.Logger.dbLogg(Companyid, 0, "FourthFilter", "air_tbo-SetAvailabilityModifiers", Supplierid, Searchid, errorMessage);
            }
        }
    }
}
