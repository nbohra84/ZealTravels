...
namespace ZealTravel.Infrastructure.Spicejet
{
    ...
    {
        public string TimeFormat(DateTime dt)
        {
        ...
        {
            string yyyy = dt.Year.ToString().Trim();
            string dd = dt.Month.ToString().Trim();
            string mm = dt.Day.ToString().Trim();

            if (dd.ToString().Length.Equals(1))
            {
                dd = "0" + dd;
            }
        ...
        public void Add_Journey_Duration_TimeDetail(DataSet dsBound)
        {
            try
            {
                ...
                foreach (DataTable table in dsBound.Tables)
                {
                    ...
                    {
                        dr["JourneyTimeDesc"] = DateTimeFormatter.ConvertTmDesc(dr["JourneyTime"].ToString());

                        if (Convert.ToInt32(dr["Duration"].ToString()) > 0)
                        {
                            dr["DurationDesc"] = DateTimeFormatter.ConvertTmDesc(dr["Duration"].ToString());
                        }
                        else
                        {
                            dr["Duration"] = DateTimeFormatter.GetDuration(dr["DepDate"].ToString(), dr["ArrDate"].ToString());
                            dr["DurationDesc"] = DateTimeFormatter.ConvertTmDesc(dr["Duration"].ToString());
                        }

                        dr["DepartureDate"] = DateTimeFormatter.DateFormat(dr["DepDate"].ToString());
                        dr["ArrivalDate"] = DateTimeFormatter.DateFormat(dr["ArrDate"].ToString());
                        dr["DepartureTime"] = DateTimeFormatter.TimeFormat(dr["DepTime"].ToString());
                        dr["ArrivalTime"] = DateTimeFormatter.TimeFormat(dr["ArrTime"].ToString());
                    }
                }
        ...
        public static string GetPromotionCodeBySupplier(string Supplierid, string PriceType)
        {

            if (Supplierid.Equals("MAAXT98402", StringComparison.OrdinalIgnoreCase)
                && (PriceType.Equals("COR", StringComparison.OrdinalIgnoreCase)
                || PriceType.Equals("CORPORATE", StringComparison.OrdinalIgnoreCase)
                || PriceType.Equals("LCC-C", StringComparison.OrdinalIgnoreCase))
                )
            {
                //dbCommon.Logger.dbLogg("", 0, "GetPromotionCodeBySupplier", "air_spicejet", Supplierid, "", "PromoCode is " + "C404MANJ");

                //return "C404MANJ";  // commented on 4-Jan-2024 if run Test credentioals have to comment this promo code
                //return "APIPROM";  // for test server but some time this also not working for test server as well
                return "";   // if you using test credentioals you have use code as empty
            }
            else
            {
                return "";
        ...
        public static string GetAvailabilityRequest(ArrayList CarrierList, string DepartureStation, string ArrivalStation, string Cabin, string StartDate, string EndDate, string Adult, string Child, string Infant)
        {
            ...
            //ReqXml.Append("<Sector>" + Sector + "</Sector>");
            //Airline_Detail objad = new Airline_Detail();
            //bool bINT = objad.IsDomestic(DepartureStation, ArrivalStation);
            //string _Sector = "";
            //if (bINT == false)
            //{
            //    _Sector = "I";
            ...
            //    _Sector = "D";
            //}
           // ReqXml.Append("<Sector>" + _Sector + "</Sector>");
        ...
        public static string GetAvailabilityRequestMC(ArrayList ArayDepartureStation, ArrayList ArayArrivalStation, ArrayList ArayStartDate, string Adult, string Child, string Infant, ArrayList CarrierList, string Cabin, string Sector)
        {
            StringBuilder ReqXml = new StringBuilder();
            ReqXml.Append("<AvailabilityRequest>");
            ReqXml.Append("<Cabin>" + Cabin + "</Cabin>");
        ...
        public static DataTable GetPassengerData(int Adt, int Chd, int Inf)
        {
            ...
            dtPassenger = Schema.SchemaPassengers;
            ...
            for (int i = 0; i < Adt; i++)
            {
                ...
                drAdd["First_Name"] = "TEST"; //"A" + RandomString(5 + i);
               
                drAdd["Last_Name"] = "TEST";
                ...
                drAdd["MealDesc_O"] = string.Empty;
                drAdd["MealCode_O"] = string.Empty;
                drAdd["MealChg_O"] = 0;
                drAdd["MealDetail_O"] = 0;

                drAdd["MealDesc_I"] = string.Empty;
                drAdd["MealCode_I"] = string.Empty;
                drAdd["MealChg_I"] = 0;
                drAdd["MealDetail_I"] = 0;

                drAdd["BaggageDesc_O"] = string.Empty;
                drAdd["BaggageCode_O"] = string.Empty;
                drAdd["BaggageChg_O"] = 0;
                drAdd["BaggageDetail_O"] = 0;

                drAdd["BaggageDesc_I"] = string.Empty;
                drAdd["BaggageCode_I"] = string.Empty;
                drAdd["BaggageChg_I"] = 0;
                drAdd["BaggageDetail_I"] = 0;
        ...
        public static DataTable RemoveZeroFares(DataTable dtBound)
        {
            ...
            DataRow[] drSelect = dtBound.Select("(Adt_BASIC + Adt_YQ +  Adt_PSF + Adt_UDF + Adt_AUDF + Adt_CUTE + Adt_GST + Adt_TF +  Adt_CESS +  Adt_EX)<1000");
        ...
        public static Int32 CalculateMealCharges(DataTable dtPassenger, string FltType)
        {
            ...
            if (FltType.Equals("O"))
            {
                ...
                if (drMeal.Length > 0)
                {
                    foreach (DataRow dr in drMeal.CopyToDataTable().Rows)
                    {
                        GetMealCharges += int.Parse(dr["MealChg_O"].ToString().Trim().ToString());
                    }
                }
            }
            else
            {
                ...
                if (drMeal.Length > 0)
                {
                    foreach (DataRow dr in drMeal.CopyToDataTable().Rows)
                    {
                        GetMealCharges += int.Parse(dr["MealChg_I"].ToString().Trim().ToString());
                    }
                }
            }
        ...
        public static Int32 CalculateBaggageCharges(DataTable dtPassenger, string FltType)
        {
            ...
            if (FltType.Equals("O"))
            {
                ...
                if (drBaggage.Length > 0)
                {
                    foreach (DataRow dr in drBaggage.CopyToDataTable().Rows)
                    {
                        GetBaggageCharges += int.Parse(dr["BaggageChg_O"].ToString().Trim().ToString());
                    }
                }
            }
            else
            {
                ...
                if (drBaggage.Length > 0)
                {
                    foreach (DataRow dr in drBaggage.CopyToDataTable().Rows)
                    {
                        GetBaggageCharges += int.Parse(dr["BaggageChg_I"].ToString().Trim().ToString());
                    }
                }
            }
        ...
        public static void WriteFileToXML(string Response, string FileName)
        {
            try
            {
                string logpath = @"D:\bProj\SG\";
                if (!Directory.Exists(logpath))
                {
                    Directory.CreateDirectory(logpath);
                }

                string filepath = logpath + FileName + ".xml";
        ...
        public static void WriteFileToTXT(string Response, string FileName)
        {
            try
            {
                ...
                string filepath = logpath + FileName + ".txt";
                System.IO.StreamWriter file = new System.IO.StreamWriter(filepath);
                file.WriteLine(Response);
                file.Close();
            }
