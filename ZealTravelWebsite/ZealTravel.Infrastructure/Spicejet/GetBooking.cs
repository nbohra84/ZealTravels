...
namespace ZealTravel.Infrastructure.Spicejet
{
    class GetBooking
    {
        ...
        //-----------------------------------------------------------------------------------------------------------------------------------------------
        public GetBooking(string Searchid, string Companyid, int BookingRef, IBookingManagementService bookingService)
        {
        ...
        //================================================================================================================================================
        public async Task<bool> GetCommitOneWay(DataTable dtBound, DataTable dtPassengerInfo, DataTable dtCompanyInfo, DataTable dtGstInfo)
        {
            ...
            try
            {
                ...
                if ((GetTotalFare + GeMealCharges + GetBaggageCharges) >= GetApiTotalFare)
                {
                    if (objGetApiLogin.GetAvailableBalance(Searchid, Signature, Supplierid) >= GetApiTotalFare)
                    {
                        ...
                        bool PaymentStatus = objbps.GetPaymentStatus(Supplierid, Signature, GetApiTotalFare);
            ...
            catch (Exception ex)
            {
               ...
               // dbCommon.Logger.dbLogg(Companyid, BookingRef, "GetCommitOneWay", "air_spicejet", Supplierid, Searchid, ex.Message + "," + ex.StackTrace);
            }
            return status;
        }
        public async Task<bool> GetCommitRT(DataTable dtOutbound, DataTable dtInbound, DataTable dtPassenger, DataTable dtCompanyInfo, DataTable dtGstInfo)
        {
            string RecordLocator = string.Empty;
            string Supplierid = string.Empty;
            string Signature = string.Empty;
            ...
            {
                errorMessage = ex.Message;
               // dbCommon.Logger.dbLogg(Companyid, BookingRef, "GetCommitRT", "air_spicejet", Supplierid, Searchid, ex.Message + "," + ex.StackTrace);
            }
            return status;
        }
    }
